...
import { BehaviorSubject, Observable, Subject, concatMap, map, of, switchMap } from 'rxjs';
...
export class UserService {
  ...
  loggedUser$: Observable<User> = this.loggedUser.asObservable().pipe(switchMap(user => {
    if (user.enabled)
      return of(user);
  ...
  private fetchUser(): Observable<User> {
    return this.localService.getData<User>(this.localService.USER_DATA).pipe(map(user => user ?? new User()),tap(user => this.loggedUser.next(user)));
  }
