...
- **Mobile** — anything under `econoflow-mobile/`
- **Docs** — `requirements.md`, `architecture.md`, `CLAUDE.md`, `README.md`, `AGENTS.md`
...
From `econoflow-mobile/`:
```bash
npm test 2>&1 | tail -40
```
Also run the TypeScript type-check (catches type errors that Jest won't):
```bash
...
## Step 3 — Run lint (when frontend or mobile files changed)
...
npm run lint 2>&1 | tail -40
```
Note: mobile lint is configured with `--max-warnings 0`, so any warning is a failure.

Record all warnings and errors for both.

---

## Step 4 — Architecture compliance review

Read every changed source file and apply the relevant checks below.
...
#### 4.9 Backend code coverage gate
...
#### 4.10 Frontend string literals & i18n
- [ ] No hard-coded user-visible strings; all strings go through `@ngx-translate`

#### 4.11 Frontend structure
- [ ] New features live under `features/<domain>/` (components, models, routes)
- [ ] Singleton services, guards, and shared UI go under `core/`
- [ ] No cross-feature imports (features must not import directly from each other)
...
#### 4.12 Mobile i18n
- [ ] No hard-coded user-visible strings; all strings use `react-i18next` (`useTranslation` hook or `i18next.t()`). Translation JSON files live in `src/i18n/locales/`.

#### 4.13 State management
- [ ] Auth and project global state lives in Zustand stores under `src/store/`
- [ ] Server/remote state uses React Query (`@tanstack/react-query`) — not ad-hoc `useState` + `useEffect` fetch patterns
- [ ] No direct API calls outside `src/api/` — all HTTP goes through the Axios instance in `src/api/client.ts` (which handles token refresh)

#### 4.14 Theming
- [ ] No hard-coded colour values (hex strings, `'red'`, etc.) — all colours come from `useAppTheme()` (which wraps react-native-paper's `useTheme` with custom tokens: `theme.colors.*` and `theme.customColors.*`)

#### 4.15 Navigation
- [ ] New screens are registered in `src/navigation/` using `@react-navigation/native-stack` or `bottom-tabs`
- [ ] No ad-hoc navigation via direct component rendering — always use the navigator

#### 4.16 Error handling
- [ ] Errors surfaced to the user use the `ErrorBanner` component (`src/components/common/ErrorBanner.tsx`), not custom inline error UI

#### 4.17 Environment variables
- [ ] API base URL is read from `EXPO_PUBLIC_API_URL` (no `/api` suffix); never hard-coded
- [ ] No other secrets or config values hard-coded in source files

#### 4.18 Mobile code coverage gate
If new logic was added, verify there are corresponding new Jest tests. Flag any exported function or component that has no test coverage.
...
- A new development command, environment variable, or configuration key → update `CLAUDE.md` (root) and/or `econoflow-mobile/AGENTS.md` if mobile-specific
...
#### Mobile
- [ ] No API keys, tokens, or secrets committed in source files or `app.json`
- [ ] Sensitive data (tokens, credentials) stored only via `expo-secure-store`, not `AsyncStorage`
- [ ] `EXPO_PUBLIC_*` env vars contain no secrets (they are baked into the JS bundle and readable by anyone)
